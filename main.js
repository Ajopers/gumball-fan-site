...
let isAdBlockEnabled = false; // Флаг для AdBlock
...
document.addEventListener('DOMContentLoaded', function() {
    detectAdBlock(); // Проверяем AdBlock перед всем
...
// Функция обнаружения AdBlock (простой хак)
function detectAdBlock() {
    ...
    setTimeout(() => {
        isAdBlockEnabled = (ads.offsetHeight === 0);
        ads.remove();
        // Если AdBlock обнаружен, можно логировать или показать предупреждение, но мы используем в loadEpisode
    }, 100);
}
...
function loadEpisode(season, episode) {
    ...
    if (embedContainer) {
        if (isAdBlockEnabled) {
            // Если AdBlock включён, показываем сообщение вместо видео
        ...
        if (episodeData) {
            ...
            console.error('Данные эпизода отсутствуют.');
        ...
        console.error('Контейнер для embed не найден.');
    }
}
